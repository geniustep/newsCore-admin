# 📰 NewsCore Admin Dashboard

<div align="center">

![NewsCore Admin](https://img.shields.io/badge/NewsCore-Admin-blue?style=for-the-badge&logo=react&logoColor=white)

[![React](https://img.shields.io/badge/React-18-61DAFB?style=flat-square&logo=react&logoColor=white)](https://reactjs.org/)
[![Vite](https://img.shields.io/badge/Vite-5-646CFF?style=flat-square&logo=vite&logoColor=white)](https://vitejs.dev/)
[![TailwindCSS](https://img.shields.io/badge/TailwindCSS-3-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white)](https://tailwindcss.com/)
[![TypeScript](https://img.shields.io/badge/TypeScript-5-3178C6?style=flat-square&logo=typescript&logoColor=white)](https://www.typescriptlang.org/)
[![TipTap](https://img.shields.io/badge/TipTap-3-000000?style=flat-square&logo=tiptap&logoColor=white)](https://tiptap.dev/)

**لوحة تحكم حديثة لنظام إدارة المحتوى الإخباري NewsCore**

[العرض التجريبي](https://newscore-admin.vercel.app) • [Backend API](https://github.com/geniustep/NewsCore)

</div>

---

## ✨ المميزات

### 🆕 النظام المتقدم (v2.0)

#### 🎨 إدارة القوالب (Theme Manager)
- **مدير قوالب متكامل** لتثبيت وإدارة القوالب
- **Theme Customizer** مع واجهة بصرية لتخصيص:
  - الألوان (أساسي، ثانوي، خلفية، نص)
  - الخطوط (عربية/إنجليزية) مع حجم الخط
  - التخطيط (عرض المحتوى، الشريط الجانبي)
  - الترويسة والتذييل
  - الصفحة الرئيسية وصفحة المقال
  - الوضع الداكن
- **معاينة حية** للتغييرات
- **استيراد/تصدير** الإعدادات

#### 🧩 إدارة الوحدات (Module Manager)
- **مدير وحدات احترافي** لتثبيت الإضافات
- **تفعيل/تعطيل** الوحدات بدون حذف
- **إدارة التبعيات** بين الوحدات
- **إعدادات مخصصة** لكل وحدة
- **فلترة** حسب النوع والحالة
- أنواع الوحدات: أساسي، إضافة، ودجت، تكامل

#### 🌍 إدارة الترجمات (Translation Manager)
- **إدارة شاملة للغات**:
  - إضافة/حذف/تعديل اللغات
  - تحديد اللغة الافتراضية
  - دعم RTL/LTR
- **نظام Namespaces** لتنظيم الترجمات
- **محرر ترجمات** مع بحث وفلترة
- **إحصائيات الاكتمال** لكل لغة
- **استيراد/تصدير** بصيغ متعددة
- **ترجمة تلقائية** بالذكاء الاصطناعي (قريباً)

### 🚨 نظام الأخبار العاجلة
- 📢 **بانر الأخبار العاجلة** مع تحديث تلقائي كل 30 ثانية
- ⚡ **إدارة كاملة** للأخبار العاجلة (إنشاء، تعديل، حذف)
- 🎯 **نظام أولويات** لترتيب الأخبار
- ⏰ **تاريخ انتهاء صلاحية** تلقائي
- 🔄 **تبديل الحالة** (نشط/غير نشط) بسهولة

### 📅 جدولة النشر
- 📆 **جدولة المقالات** للنشر في تاريخ ووقت محدد
- 📋 **صفحة مخصصة** لعرض جميع المقالات المجدولة
- ⏰ **تذكيرات بصرية** للمقالات المجدولة

### 🔍 SEO متقدم
- 📊 **محرر SEO شامل** مع تبويبات منظمة
- 📱 **معاينة Facebook (Open Graph)** مباشرة
- 🐦 **معاينة Twitter Cards** مباشرة
- 🏷️ **محرر Schema.org** لتحسين نتائج البحث
- 📈 **محلل SEO** يقدم اقتراحات وتحذيرات
- ✏️ **تحكم كامل** في العنوان، الوصف، الكلمات المفتاحية

### 📝 محرر متقدم (TipTap Editor)
- ✍️ **محرر نصوص قوي** مبني على TipTap
- 💾 **حفظ تلقائي** مع مؤشر حالة (محفوظ/جاري الحفظ)
- 👁️ **معاينة فورية** للمقالات أثناء التحرير
- 📱 **معاينة متعددة الأجهزة** (ديسكتوب/تابلت/موبايل)
- 📋 **وضع HTML** لتحرير الكود مباشرة
- 🗂️ **عرض هيكل المستند** مثل WordPress
- 🎨 **تنسيق نصوص متقدم**:
  - ألوان النصوص والخلفيات
  - محاذاة النصوص (يمين/وسط/يسار)
  - قوائم مرتبة وغير مرتبة
  - روابط وصور
  - اقتباسات وأكواد برمجية
  - جداول وخطوط أفقية
- 🔗 **رابط مباشر للمقال** داخل المحرر
- 📂 **إخفاء/إظهار الشريط الجانبي** أثناء التحرير

### 🎨 إدارة القالب الشاملة
- 🖼️ **إدارة الشعارات**:
  - رفع شعار منفصل لكل لغة (عربي/إنجليزي/فرنسي)
  - رفع Favicon مخصص
  - معاينة مباشرة للشعارات
- 🌈 **تخصيص الألوان**:
  - 6 قوالب ألوان جاهزة
  - محرر ألوان مخصص (5 ألوان رئيسية)
  - ألوان منفصلة للوضع الليلي
- 🔤 **إدارة الخطوط**:
  - 6 خيارات خطوط عربية
  - تحكم بحجم الخط
  - خط منفصل للعناوين
- 📐 **تخصيص التصميم**:
  - تحكم بـ Border Radius
  - تحكم بالمسافات (Spacing)
- 🌙 **الوضع الليلي** مع دعم كامل
- 💾 **استيراد/تصدير** إعدادات القالب
- 👁️ **معاينة حية** للتغييرات
- ⚡ **تطبيق فوري** على جميع عناصر التطبيق

### 🔄 Editorial Workflow (سير العمل التحريري)
- 📋 **نظام حالات متقدم** (مسودة، قيد المراجعة، معتمد، منشور)
- 👥 **تعيين مراجعين** للمقالات
- 💬 **نظام تعليقات** للمراجعة مع إمكانية الرد
- 📊 **خط زمني** يوضح تاريخ التغييرات
- ✅ **شارات حالة** واضحة لكل مرحلة

### 📊 نظام التحليلات المتقدم
- 📈 **لوحة تحليلات شاملة** مع إحصائيات مباشرة
- 👁️ **مشاهدات الصفحة** مع رسوم بيانية
- 👥 **إحصائيات الزوار** ومصادر الزيارات
- ⏱️ **متوسط وقت القراءة** ومعدل الارتداد
- 🔥 **المقالات الأكثر مشاهدة**
- 📊 **مصادر الزيارات** (مباشر، بحث، اجتماعي)
- ⚡ **الزوار المباشرون** مع تحديث كل 5 ثوان

### 🧩 محرر البلوكات (Block Editor)
- 🎨 **8 أنواع بلوكات** مختلفة:
  - 📝 نص
  - 🖼️ صورة
  - 🖼️ معرض صور
  - 🎥 فيديو
  - 💬 اقتباس
  - 💻 كود برمجي
  - 🔗 تضمين
  - 📊 جدول
- 🔄 **ترتيب البلوكات** بسهولة (أعلى/أسفل)
- ✏️ **تحرير مباشر** لكل بلوك
- 🗑️ **حذف سريع** للبلوكات

### 🎨 **واجهة حديثة** مبنية بـ React + TailwindCSS
### 🌐 **دعم كامل للعربية** مع تصميم RTL
### 📁 **إدارة التصنيفات والوسوم**
### 🖼️ **إدارة الوسائط** مع رفع الملفات
### 👥 **إدارة المستخدمين** والصلاحيات
### 🔐 **نظام مصادقة** آمن مع JWT
### 📱 **تصميم متجاوب** لجميع الأجهزة

---

## 🛠️ التقنيات المستخدمة

### Frontend Framework
- **React 18** - مكتبة بناء واجهات المستخدم
- **TypeScript** - لغة برمجة مع أنواع ثابتة
- **Vite** - أداة بناء سريعة

### UI & Styling
- **TailwindCSS** - إطار عمل CSS
- **Heroicons** - مكتبة أيقونات
- **IBM Plex Sans Arabic** - خط عربي احترافي

### State Management
- **Zustand** - إدارة الحالة الخفيفة
- **React Context** - إدارة القالب العامة
- **TanStack Query (React Query)** - إدارة حالة الخادم

### Editor & Forms
- **TipTap** - محرر نصوص متقدم
- **React Hook Form** - إدارة النماذج
- **React Hot Toast** - إشعارات المستخدم

### Development Tools
- **ESLint** - فحص جودة الكود
- **PostCSS** - معالج CSS
- **Autoprefixer** - إضافة بادئات CSS تلقائياً

---

## 🚀 التثبيت

### المتطلبات

- Node.js >= 20.x
- npm >= 10.x

### خطوات التثبيت

```bash
# استنساخ المشروع
git clone https://github.com/geniustep/newsCore-admin.git
cd newsCore-admin

# تثبيت التبعيات
npm install

# تشغيل في وضع التطوير
npm run dev
```

---

## ⚙️ الإعداد

### متغيرات البيئة

أنشئ ملف `.env.local` في جذر المشروع:

```env
# API URL - رابط Backend API
# في الإنتاج على Vercel: اتركه فارغاً إذا كان Backend على نفس النطاق
# في التطوير المحلي: اتركه فارغاً لاستخدام Proxy من vite.config.ts
VITE_API_URL=
```

**ملاحظة مهمة:**
- في **الإنتاج على Vercel**: إذا كان الـ Backend على نفس النطاق (`admin.sahara2797.com`)، اترك `VITE_API_URL` فارغاً. الكود سيستخدم مسارات نسبية (`/api/v1`) تلقائياً.
- في **التطوير المحلي**: اترك `VITE_API_URL` فارغاً لاستخدام Proxy المحدد في `vite.config.ts`.
- إذا كان الـ Backend على نطاق مختلف، أضف الرابط الكامل هنا.

---

## 🏃 التشغيل

```bash
# وضع التطوير
npm run dev

# بناء للإنتاج
npm run build

# معاينة البناء
npm run preview
```

---

## 🚢 النشر على Vercel و Cloudflare

### 📦 النشر على Vercel

#### 1. ربط المستودع

1. اذهب إلى [Vercel Dashboard](https://vercel.com/dashboard)
2. اضغط "Add New Project"
3. اختر مستودع `geniustep/newsCore-admin` أو ربط مستودع GitHub الخاص بك

#### 2. إعداد المتغيرات

**⚠️ مهم:** لا تحتاج إلى إضافة `VITE_API_URL` في Vercel إذا كان الـ Backend على نفس النطاق.

الكود يستخدم تلقائياً مسارات نسبية (`/api/v1`) في الإنتاج لتجنب حلقة إعادة التوجيه (508 Loop Detected).

**إذا كان الـ Backend على نطاق مختلف:**
1. اذهب إلى **Settings** → **Environment Variables**
2. أضف المتغير `VITE_API_URL` مع رابط الـ Backend الكامل
3. اختر البيئات (Production, Preview, Development)
4. احفظ التغييرات

**ملاحظة:** إذا كان الـ Frontend والـ Backend على نفس النطاق (مثل `admin.sahara2797.com`)، اترك `VITE_API_URL` فارغاً أو لا تضيفه - الكود سيستخدم المسارات النسبية تلقائياً.

#### 3. إعدادات البناء

Vercel سيكتشف تلقائياً:
- **Framework Preset**: Vite
- **Build Command**: `npm run build`
- **Output Directory**: `dist`
- **Install Command**: `npm install`

#### 4. النشر

بعد ربط المستودع:
- Vercel سيقوم تلقائياً بنشر كل commit جديد على branch `main`
- ستحصل على رابط مثل: `https://your-project.vercel.app`

---

### ☁️ ربط Cloudflare مع Vercel

#### الطريقة 1: استخدام Cloudflare كـ DNS فقط (مُوصى به)

هذه الطريقة تحافظ على Vercel كـ Hosting وتحسن الأداء عبر Cloudflare CDN:

##### الخطوة 1: إعداد النطاق في Vercel

1. اذهب إلى **Settings** → **Domains** في مشروع Vercel
2. أضف النطاق المخصص (مثل: `admin.yourdomain.com`)
3. Vercel سيعطيك سجلات DNS المطلوبة

##### الخطوة 2: إعداد Cloudflare DNS

1. اذهب إلى [Cloudflare Dashboard](https://dash.cloudflare.com)
2. اختر النطاق الخاص بك
3. اذهب إلى **DNS** → **Records**
4. أضف السجلات التالية:

| النوع | الاسم | المحتوى | TTL | Proxy |
|------|------|---------|-----|-------|
| `CNAME` | `admin` | `cname.vercel-dns.com` | Auto | 🟠 Proxied |
| `CNAME` | `www.admin` | `cname.vercel-dns.com` | Auto | 🟠 Proxied |

**ملاحظة:** استخدم القيمة الصحيحة من Vercel (قد تكون مختلفة)

##### الخطوة 3: تفعيل Cloudflare Features

في Cloudflare Dashboard:

1. **SSL/TLS** → **Overview**
   - اختر **Full (strict)** mode
   - هذا يضمن تشفير كامل بين Cloudflare و Vercel

2. **Speed** → **Optimization**
   - فعّل **Auto Minify** (HTML, CSS, JS)
   - فعّل **Brotli** compression

3. **Caching** → **Configuration**
   - **Caching Level**: Standard
   - **Browser Cache TTL**: 4 hours
   - **Edge Cache TTL**: 1 month (للملفات الثابتة)

4. **Network**
   - فعّل **HTTP/2**
   - فعّل **HTTP/3 (with QUIC)**
   - فعّل **0-RTT Connection Resumption**

##### الخطوة 4: إعداد Page Rules (اختياري)

لتحسين الأداء أكثر:

1. اذهب إلى **Rules** → **Page Rules**
2. أنشئ قاعدة جديدة:
   - **URL Pattern**: `admin.yourdomain.com/assets/*`
   - **Settings**:
     - Cache Level: Cache Everything
     - Edge Cache TTL: 1 month
     - Browser Cache TTL: 1 month

---

#### الطريقة 2: استخدام Cloudflare Pages (بديل)

إذا كنت تريد استخدام Cloudflare Pages بدلاً من Vercel:

##### الخطوة 1: ربط المستودع

1. اذهب إلى [Cloudflare Pages](https://pages.cloudflare.com)
2. اضغط **Create a project**
3. اختر **Connect to Git**
4. اختر مستودع GitHub الخاص بك

##### الخطوة 2: إعدادات البناء

| الإعداد | القيمة |
|---------|--------|
| **Framework preset** | Vite |
| **Build command** | `npm run build` |
| **Build output directory** | `dist` |
| **Root directory** | `/` (أو المسار الصحيح) |

##### الخطوة 3: متغيرات البيئة

أضف في **Environment variables**:

| المتغير | القيمة |
|---------|--------|
| `VITE_API_URL` | `https://admin.sahara2797.com/api/v1` |
| `NODE_VERSION` | `20` |

##### الخطوة 4: إعدادات Cloudflare

1. **Custom domains**: أضف النطاق المخصص
2. **Functions**: (اختياري) لإضافة Serverless Functions
3. **Analytics**: فعّل لمراقبة الأداء

---

### 🔧 إعدادات إضافية

#### إعدادات Vercel (vercel.json)

المشروع يحتوي بالفعل على ملف `vercel.json` مع الإعدادات التالية:

- ✅ Rewrites للـ API routes
- ✅ Headers للأمان
- ✅ Cache headers للملفات الثابتة

#### إعدادات Cloudflare Workers (اختياري)

يمكنك إنشاء Worker لتحسين الأداء:

```javascript
// cloudflare-worker.js
addEventListener('fetch', event => {
  event.respondWith(handleRequest(event.request))
})

async function handleRequest(request) {
  // إضافة headers للأمان
  const response = await fetch(request)
  const newResponse = new Response(response.body, response)
  
  newResponse.headers.set('X-Content-Type-Options', 'nosniff')
  newResponse.headers.set('X-Frame-Options', 'DENY')
  newResponse.headers.set('X-XSS-Protection', '1; mode=block')
  
  return newResponse
}
```

---

### ✅ التحقق من الإعداد

بعد إكمال الخطوات:

1. **تحقق من SSL**: تأكد أن الموقع يعمل على `https://`
2. **تحقق من الأداء**: استخدم [PageSpeed Insights](https://pagespeed.web.dev/)
3. **تحقق من CDN**: تأكد أن الملفات الثابتة يتم تحميلها من Cloudflare

---

### 🐛 حل المشاكل الشائعة

#### مشكلة: الموقع لا يعمل بعد ربط Cloudflare

**الحل:**
- تأكد من أن SSL mode في Cloudflare هو **Full (strict)**
- تحقق من أن السجلات DNS صحيحة
- انتظر 5-10 دقائق لتحديث DNS

#### مشكلة: API calls لا تعمل أو خطأ 508 (Loop Detected)

**الحل:**
- **لا تضيف** `VITE_API_URL` في Vercel إذا كان الـ Backend على نفس النطاق
- الكود يستخدم تلقائياً مسارات نسبية في الإنتاج لتجنب حلقة إعادة التوجيه
- تحقق من إعدادات CORS في Backend
- تأكد من أن `vercel.json` لا يحتوي على rewrite rules تسبب حلقة (تم إزالتها في الإصدار الحالي)
- إذا كان الـ Backend على نطاق مختلف، أضف `VITE_API_URL` مع الرابط الكامل

#### مشكلة: الملفات الثابتة لا يتم cache

**الحل:**
- تحقق من إعدادات Cache في Cloudflare
- تأكد من headers في `vercel.json`
- راجع Page Rules في Cloudflare

---

### 📊 مراقبة الأداء

#### Cloudflare Analytics

1. اذهب إلى **Analytics** → **Web Analytics**
2. فعّل Web Analytics للموقع
3. راقب:
   - عدد الزوار
   - سرعة التحميل
   - معدل الأخطاء

#### Vercel Analytics

1. اذهب إلى **Analytics** في Vercel Dashboard
2. فعّل Vercel Analytics
3. راقب:
   - Core Web Vitals
   - Real User Monitoring

---

## 📁 بنية المشروع

```
newsCore-admin/
├── 📂 src/
│   ├── 📂 components/         # المكونات المشتركة
│   │   ├── Layout.tsx         # التخطيط الرئيسي مع القالب الديناميكي
│   │   ├── TipTapEditor.tsx   # محرر TipTap المتقدم
│   │   ├── DocumentOutline.tsx # عرض هيكل المستند
│   │   ├── ArticlePreviewModal.tsx # معاينة المقالات
│   │   ├── SchedulePublisher.tsx # جدولة النشر
│   │   ├── 📂 BreakingNews/  # نظام الأخبار العاجلة
│   │   │   ├── BreakingNewsBanner.tsx
│   │   │   ├── BreakingNewsManager.tsx
│   │   │   └── BreakingNewsEditor.tsx
│   │   ├── 📂 SEO/           # محرر SEO المتقدم
│   │   │   ├── SEOEditor.tsx
│   │   │   ├── OpenGraphPreview.tsx
│   │   │   ├── TwitterCardPreview.tsx
│   │   │   ├── SchemaEditor.tsx
│   │   │   └── SEOAnalyzer.tsx
│   │   ├── 📂 Workflow/      # سير العمل التحريري
│   │   │   ├── StatusBadge.tsx
│   │   │   ├── WorkflowTimeline.tsx
│   │   │   ├── ReviewComments.tsx
│   │   │   └── AssignReviewer.tsx
│   │   ├── 📂 Analytics/     # نظام التحليلات
│   │   │   ├── AnalyticsCard.tsx
│   │   │   ├── ChartLine.tsx
│   │   │   ├── TopArticles.tsx
│   │   │   ├── TrafficSources.tsx
│   │   │   └── RealtimeVisitors.tsx
│   │   └── 📂 BlockEditor/  # محرر البلوكات
│   │       ├── BlockEditor.tsx
│   │       ├── BlockToolbar.tsx
│   │       ├── BlockRenderer.tsx
│   │       └── 📂 blocks/
│   │           ├── TextBlock.tsx
│   │           ├── ImageBlock.tsx
│   │           ├── GalleryBlock.tsx
│   │           ├── VideoBlock.tsx
│   │           ├── QuoteBlock.tsx
│   │           ├── CodeBlock.tsx
│   │           ├── EmbedBlock.tsx
│   │           └── TableBlock.tsx
│   ├── 📂 pages/              # صفحات التطبيق
│   │   ├── Login.tsx          # تسجيل الدخول
│   │   ├── Dashboard.tsx      # لوحة التحكم
│   │   ├── Articles.tsx       # المقالات
│   │   ├── ArticleEditor.tsx  # محرر المقالات مع الحفظ التلقائي
│   │   ├── ArticleWorkflow.tsx # سير العمل التحريري
│   │   ├── BreakingNews.tsx   # الأخبار العاجلة
│   │   ├── ScheduledPosts.tsx  # المقالات المجدولة
│   │   ├── Analytics.tsx      # التحليلات
│   │   ├── Categories.tsx     # التصنيفات
│   │   ├── Tags.tsx           # الوسوم
│   │   ├── Media.tsx          # الوسائط
│   │   ├── Users.tsx          # المستخدمين
│   │   ├── ThemeSettings.tsx  # إعدادات القالب
│   │   ├── Settings.tsx       # الإعدادات
│   │   ├── 🆕 ThemeManager.tsx    # إدارة القوالب
│   │   ├── 🆕 ModuleManager.tsx   # إدارة الوحدات
│   │   └── 🆕 TranslationManager.tsx # إدارة الترجمات
│   ├── 📂 types/
│   │   └── workflow.ts        # أنواع سير العمل
│   ├── 📂 contexts/
│   │   └── ThemeContext.tsx   # إدارة القالب العامة
│   ├── 📂 lib/
│   │   └── api.ts             # API Client (محدث)
│   ├── 📂 store/
│   │   └── auth.ts            # إدارة حالة المصادقة
│   ├── App.tsx                # المكون الرئيسي (محدث)
│   ├── main.tsx               # نقطة الدخول
│   └── index.css              # الأنماط العامة + TipTap
├── 📄 index.html
├── 📄 package.json
├── 📄 vite.config.ts
├── 📄 tailwind.config.js
├── 📄 tsconfig.json
├── 📄 vercel.json             # إعدادات Vercel
└── 📄 IMPLEMENTATION_ROADMAP.md # خارطة الطريق التنفيذية
```

---

## 🆕 الميزات الجديدة (v2.0)

تم إضافة الميزات التالية في الإصدار 2.0:

- ✅ نظام الأخبار العاجلة الكامل
- ✅ جدولة النشر
- ✅ SEO متقدم مع معاينات
- ✅ Editorial Workflow (سير العمل التحريري)
- ✅ نظام التحليلات الشامل
- ✅ محرر البلوكات مع 8 أنواع مختلفة

### 🆕 الميزات الجديدة (v2.1) - النظام المتقدم

تم إضافة نظام متقدم يجعل NewsCore منصة قابلة للتوسعة مثل WordPress:

- ✅ **نظام القوالب (Themes)** - تثبيت وتخصيص القوالب
- ✅ **نظام الوحدات (Modules/Plugins)** - إضافات قابلة للتثبيت
- ✅ **نظام الترجمة (i18n)** - إدارة شاملة للغات والترجمات
- ✅ **نظام الـ Hooks** - أحداث قابلة للتوسعة
- ✅ **نظام الودجات** - إدارة ودجات الموقع

للمزيد من التفاصيل، راجع:
- [IMPLEMENTATION_ROADMAP.md](./IMPLEMENTATION_ROADMAP.md)
- [ADVANCED_SYSTEM.md](../docs/ADVANCED_SYSTEM.md)

---

## 🔗 الروابط

- **Backend API**: [NewsCore](https://github.com/geniustep/NewsCore)
- **العرض التجريبي**: https://admin.sahara2797.com
- **توثيق API**: https://admin.sahara2797.com/api/docs

---

## 📝 بيانات الدخول التجريبية

```
البريد: admin@sahara2797.com
كلمة المرور: Admin@123456
```

---

## 📄 الترخيص

MIT License

---

<div align="center">

**صُنع بـ ❤️ للمجتمع العربي**

</div>

